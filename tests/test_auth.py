...
class TestExchangeCodeForTokens:
    ...
    def test_exchange_code_missing_client_id(self):
        ...
        with patch.dict('os.environ', {}, clear=True):
            with pytest.raises(AuthenticationError, match="RESTREAM_CLIENT_ID environment variable not set"):
    ...
    def test_exchange_code_server_error(self):
        ...
        responses.add(
            responses.POST,
            "https://api.restream.io/oauth/token",
            json={"error": "invalid_grant", "error_description": "The authorization code is invalid"},
            status=400
        )
        
        with patch.dict('os.environ', {'RESTREAM_CLIENT_ID': 'test-client-id'}):
            with pytest.raises(AuthenticationError, match="Token exchange failed: 400 - The authorization code is invalid"):
                auth.exchange_code_for_tokens(
    ...
    def test_exchange_code_network_error(self):
        ...
        with patch.dict('os.environ', {'RESTREAM_CLIENT_ID': 'test-client-id'}):
            with patch('requests.post', side_effect=requests.exceptions.ConnectionError("Network error")):
                with pytest.raises(AuthenticationError, match="Network error during token exchange"):
                    auth.exchange_code_for_tokens(
                        auth_code="test-code",
                        redirect_uri="http://localhost:8080/callback"
                    )
...
class TestPerformLogin:
    ...
    def test_perform_login_builds_correct_url(self, mock_server, mock_browser, temp_config_dir):
    ...
    @patch('restream_io.auth.webbrowser.open')
    @patch('restream_io.auth.HTTPServer')
    def test_perform_login_without_pkce(self, mock_server, mock_browser, temp_config_dir):
    ...
    @patch('restream_io.auth.webbrowser.open')
    @patch('restream_io.auth.HTTPServer')
    def test_perform_login_success_flow(self, mock_server, mock_browser, temp_config_dir):
