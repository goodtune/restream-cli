...
class TestAPIError:
    ...
    def test_api_error_with_all_details(self):
        ...
        error = APIError(
            ...
            url="https://api.restream.io/v1/profile"
        )
        
        expected = "Request failed | Status: 500 | URL: https://api.restream.io/v1/profile | Response: Internal server error"
        assert str(error) == expected
        assert error.status_code == 500
        assert error.response_text == "Internal server error"
        assert error.url == "https://api.restream.io/v1/profile"
...
class TestRestreamClientEnhanced:
    ...
    def test_make_request_error_with_json_response(self):
        ...
        error_response = {"error": "invalid_request", "message": "Missing required parameter"}
        responses.add(
            "GET", 
            "https://api.restream.io/v1/test", 
            json=error_response, 
        ...
        assert "Missing required parameter" in error.message  # Using message field directly
        assert error.url == "https://api.restream.io/v1/test"
    ...
    def test_from_config_expired_token_with_refresh(self, mock_refresh, mock_load_tokens):
...
class TestRetryLogic:
    ...
    def test_retry_on_transient_error(self):
        ...
        responses.add("GET", "https://api.restream.io/v1/profile", json={"id": 123, "username": "test", "email": "test@example.com"}, status=200)
    ...
    def test_no_retry_on_non_transient_error(self):
        ...
        responses.add("GET", "https://api.restream.io/v1/profile", status=404)
