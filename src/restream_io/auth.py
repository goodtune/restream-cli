...
def exchange_code_for_tokens(auth_code: str, redirect_uri: str, code_verifier: Optional[str] = None) -> dict:
...
def perform_login(client_id: str = None, redirect_port: int = None, use_pkce: bool = True) -> bool:
    ...
    Args:
        ...
        redirect_port: Port for local redirect server (auto-assigned if not provided)
    ...
    if not client_id:
        raise AuthenticationError("Client ID not provided and RESTREAM_CLIENT_ID environment variable not set")
    ...
    auth_url = "https://api.restream.io/oauth/authorize?" + urllib.parse.urlencode(auth_params)
